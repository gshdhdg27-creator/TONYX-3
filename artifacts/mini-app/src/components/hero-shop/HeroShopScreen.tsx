...
function MageCard({ mage, owned, equipped, selectMode, onBuy, onSelect }: MageCardProps) {
  const rs = RARITY_STYLE[mage.rarity];
  const free = mage.priceTon === 0;

  // In select mode: owned cards show "Выбрать" / equipped show "В слоте"
  // In shop mode: owned show "✓ Есть" disabled / unowned show buy button
  const renderButton = () => {
    if (selectMode) {
      if (!owned) {
        return (
          <button
            ...
            style={{
              ...
              letterSpacing: 0.3, boxShadow: "0 2px 12px rgba(14,165,233,0.3)", transition: "opacity 0.15s",
            }}
          >
            {free ? "Получить бесплатно" : `Купить ${mage.priceTon} TON`}
          </button>
        );
      }
      if (equipped) {
        return (
          <button
            ...
            style={{
              ...
              fontSize: 12, fontWeight: 800, cursor: "default", letterSpacing: 0.3,
            }}
          >
            ⚔️ В слоте
          </button>
      ...
      return (
        <button
          ...
          style={{
            ...
            color: "#fff", fontSize: 12, fontWeight: 800, cursor: "pointer",
            letterSpacing: 0.3, boxShadow: "0 2px 12px rgba(16,185,129,0.35)", transition: "opacity 0.15s",
          }}
        >
          ✔ Выбрать
        </button>
    ...
    return (
      <button
        ...
        style={{
          ...
          fontSize: 12, fontWeight: 800, cursor: owned ? "default" : "pointer",
          letterSpacing: 0.3,
          boxShadow: owned ? "none" : "0 2px 12px rgba(14,165,233,0.3)",
          transition: "opacity 0.15s",
        }}
      >
        {owned ? "✓ Уже куплен" : free ? "Получить бесплатно" : `${mage.priceTon} TON`}
      </button>
    );
  };
    ...
    <div style={{
      ...
      <div style={{ position: "relative", aspectRatio: "3/4", overflow: "hidden" }}>
        ...
        <div style={{ position: "absolute", top: 8, left: 0, right: 0, display: "flex", justifyContent: "center" }}>
          <div style={{
            background: rs.badge, color: "#fff", fontSize: 9, fontWeight: 900,
        ...
        {owned && (
          <div style={{
            ...
            background: equipped ? "rgba(251,191,36,0.92)" : "rgba(16,185,129,0.92)",
        ...
        <div style={{
          ...
          <div style={{
            ...
            textAlign: "center", lineHeight: 1.2, textShadow: "0 1px 4px rgba(0,0,0,0.8)",
          }}>
            {mage.name}
          </div>
        </div>
      ...
      <div style={{
        ...
        padding: "8px 6px 6px", borderTop: `1px solid ${rs.border}`,
        background: "rgba(255,255,255,0.03)", gap: 4,
      }}>
...
const FILTERS: { key: "all" | "owned" | "rare" | "epic" | "legendary"; label: string }[] = [
  { key: "all", label: "Все" },
  { key: "owned", label: "Куплено" },
  { key: "rare", label: "Rare" },
  { key: "epic", label: "Epic" },
...
export default function HeroShopScreen() {
  ...
  const pendingSlotIndex = useGameStore((s) => s.pendingSlotIndex);
  const buyMage = useGameStore((s) => s.buyMage);
  const equipMageToSlot = useGameStore((s) => s.equipMageToSlot);

  const [filter, setFilter] = useState<"all" | "owned" | "rare" | "epic" | "legendary">(
    // If opened from a slot tap, default to "Куплено" tab
    pendingSlotIndex !== null ? "owned" : "all"
  );

  const selectMode = pendingSlotIndex !== null;

  const ownedIds = new Set(ownedMages.map((m) => m.id));
  const equippedIds = new Set(equippedSlots.filter(Boolean) as string[]);

  const visible = (() => {
    if (filter === "owned") return MAGES.filter((m) => ownedIds.has(m.id));
    if (filter === "all") return MAGES;
    return MAGES.filter((m) => m.rarity === filter);
  })();

  const handleBack = () => {
    // Clear pending slot and go back
    useGameStore.setState({ pendingSlotIndex: null });
    setView("home");
  };
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        marginTop: 56, flexShrink: 0, background: "#08080f",
      }}>
        <button
          onClick={handleBack}
          style={{
            background: "rgba(255,255,255,0.06)", border: "none", borderRadius: 10,
            padding: "8px 14px", color: "#a0a0c0", fontSize: 14,
            fontWeight: 700, cursor: "pointer", marginRight: 12,
          }}
        >
          ←
        </button>
        <div style={{ flex: 1 }}>
          <div style={{ fontSize: 17, fontWeight: 900, color: "#f1f1f8", letterSpacing: 0.5 }}>
            {selectMode
              ? `⚔️ Слот ${(pendingSlotIndex ?? 0) + 1} — выбери мага`
              : "⚔️ Магазин магов"}
          </div>
          <div style={{ fontSize: 11, color: "#606080", marginTop: 2 }}>
            {selectMode
              ? "Выбери купленного мага или купи нового"
      ...
      {selectMode && (
        <div style={{
          ...
          Нажми «Выбрать» под купленным магом — он встанет в слот
        </div>
      )}
      ...
      <div style={{
        ...
        flexShrink: 0, background: "#08080f", overflowX: "auto",
      }}>
        {FILTERS.map((f) => {
          const active = filter === f.key;
          const rs = (f.key !== "all" && f.key !== "owned") ? RARITY_STYLE[f.key as MageRarity] : null;
          return (
            <button
              key={f.key}
            ...
            >
              {f.label}
              {f.key === "owned" && ownedIds.size > 0 && (
                <span style={{ marginLeft: 4, opacity: 0.7 }}>({ownedIds.size})</span>
              )}
            </button>
          );
        })}
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "12px 12px 32px" }}>
        {visible.length === 0 ? (
          <div style={{
            ...
            <div style={{ fontSize: 14, color: "#606080", fontWeight: 700 }}>
              {filter === "owned" ? "Нет купленных магов" : "Нет магов"}
            </div>
            {filter === "owned" && (
              <button
                onClick={() => setFilter("all")}
                style={{
                  marginTop: 4, padding: "9px 20px", borderRadius: 12,
