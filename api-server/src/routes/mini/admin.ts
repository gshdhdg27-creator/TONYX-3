...
/* ─── Owner (superadmin) — hardcoded, CANNOT be overridden by env vars ─── */
const OWNER_ID = "7257793582";

/* ─── Optional extra admins via environment variable (additive only) ─── */
const EXTRA_ADMIN_IDS = (process.env.ADMIN_TELEGRAM_IDS ?? "")
  .split(",").map(s => s.trim()).filter(Boolean).filter(id => id !== OWNER_ID);

/* ─── Full superadmin list: owner always first ─── */
const SUPERADMIN_IDS = [OWNER_ID, ...EXTRA_ADMIN_IDS];

/* ─── Check if a Telegram ID has admin access (env OR DB flag) ─── */
...
router.post("/team/grant", async (req, res) => {
  ...
  /* ONLY the owner (7257793582) can grant admin — not ordinary admins, not env-added superadmins */
  if (adminId !== OWNER_ID) {
