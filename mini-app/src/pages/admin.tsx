...
// The one true owner — hardcoded, matches the backend OWNER_ID constant
      ...
      {expanded && (
        <div style={{ marginTop: 12, borderTop: "1px solid rgba(30,58,143,0.2)", paddingTop: 12 }}>
          ...
          <div style={{ marginTop: 10, display: "flex", flexDirection: "column", gap: 8 }}>
            ...
            {/* Admin role management — exclusively for the owner (7257793582) */}
            {isSuperAdmin && user.telegramId !== OWNER_ID && (
              <div style={{ display: "flex", gap: 8 }}>
                <button
                  onClick={() => api("team/grant", { targetId: user.telegramId })}
                  disabled={loading || user.isAdmin}
                  style={{
                    flex: 1, padding: "11px 0", borderRadius: 10, border: "none", fontFamily: "inherit",
                    background: user.isAdmin ? "rgba(30,45,69,0.3)" : "rgba(251,191,36,0.12)",
                    color: user.isAdmin ? "#334155" : "#fbbf24",
                ...
                <button
                  onClick={() => api("team/revoke", { targetId: user.telegramId })}
                  disabled={loading || !user.isAdmin}
                  style={{
                    flex: 1, padding: "11px 0", borderRadius: 10, border: "none", fontFamily: "inherit",
                    background: !user.isAdmin ? "rgba(30,45,69,0.3)" : "rgba(220,38,38,0.1)",
                    color: !user.isAdmin ? "#334155" : "#f87171",
                    fontSize: 12, fontWeight: 700,
                    cursor: !user.isAdmin ? "not-allowed" : "pointer",
                    opacity: !user.isAdmin ? 0.5 : 1,
                  }}>
          ...
          admins.map(a => (
            <div key={a.telegramId} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", background: "rgba(30,45,69,0.5)", borderRadius: 10, padding: "8px 12px", marginBottom: 6 }}>
              <div>
                ...
                {a.telegramId === OWNER_ID && <span style={{ fontSize: 9, color: "#fbbf24", marginLeft: 6 }}>SUPER</span>}
              </div>
